...
_gaq.push([ "_setAccount", "UA-71812070-2" ]), _gaq.push([ "_setSampleRate", "2.5" ]), 
function() {
...
try {
    ...
    var isEdge = navigator.userAgent.toLowerCase().indexOf("edg") > -1, browser = isEdge ? "edge" : "chrome";
    ...
    chrome.storage.local.get(null, (function(data) {
        data.userId ? data.recentlyUpdated3 ? (console.log("chrome storage local has user id: " + data.userId), 
        chrome.runtime.setUninstallURL("https://www.netflixparty.com/uninstall?userId=" + data.userId)) : (console.log("userId not recently updated 3 -> now resetting"), 
        data.oldUserId ? resetUserId3(data.userId, data.oldUserId) : resetUserId3(data.userId)) : (console.log("userId undefined in local storage -> now setting"), 
        setUserId());
    }));
} catch (e) {
...
chrome.runtime.onMessage.addListener((function(request, sender, sendResponse) {
    if (console.log(sender.tab ? "from a content script:" + sender.tab.url : "from the extension"), 
    request.summary) {
        var xmlhttp = new XMLHttpRequest;
        xmlhttp.open("POST", "https://data3.netflixparty.com/log-summary", !0), xmlhttp.setRequestHeader("Content-Type", "application/json"), 
        xmlhttp.send(JSON.stringify(request.summary)), sendResponse({
            farewell: "goodbye"
        });
    } else if ("getIconMap" == request.type) {
        var iconMap = {};
        return chrome.runtime.getPackageDirectoryEntry((async root => {
            root.getDirectory("img/icons/", null, (dir => {
                dir.createReader().readEntries((async results => {
                    if (results.length) {
                        const iconPromises = results.map((async category => processIconsAsync(category, iconMap)), (err => {}));
