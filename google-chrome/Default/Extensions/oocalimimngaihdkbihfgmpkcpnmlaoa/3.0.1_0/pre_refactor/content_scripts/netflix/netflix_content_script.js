var injectContentScript = function() {
    ...
    chrome.storage.local.get([ "userId" ], (function(data) {
        data.userId && (permId = data.userId), permIdFix && (permId = userId);
    }));
    ...
    var injectScript = function(script) {
        ...
        return "https://www.tele.pe/netflix/" + encodeURIComponent(sessionId) + "?s=" + encodeURIComponent(serverId);
    }, uiEventsHappening = 0, lastDuration = 36e5, getDuration = function() {
        var video = jQuery("video");
        return video.length > 0 && (lastDuration = Math.floor(1e3 * video[0].duration)), 
        lastDuration;
    }, getState = function() {
        return jQuery(".legacy-controls-styles.legacy.dimmed").length > 0 ? "idle" : jQuery(".AkiraPlayerSpinner--container").length > 0 ? "loading" : jQuery(".button-nfplayerPause").length > 0 ? "playing" : "paused";
    }, getPlaybackPosition = function() {
        return jQuery("video")[0] ? Math.floor(1e3 * jQuery("video")[0].currentTime) : null;
    }, getRemainingTime = function() {
        return jQuery("video")[0] ? Math.floor(1e3 * (jQuery("video")[0].duration - jQuery("video")[0].currentTime)) : null;
        ...
        if (jQuery(".time-remaining__time")[0]) {
            ...
            if ("0:00" !== remainingTimeText) return remainingTimeText;
        }
        return null;
    }, wakeUp = function() {
        uiEventsHappening += 1;
        var idleDisplay = jQuery(".legacy-controls-styles.legacy.dimmed"), eventOptions = {
            bubbles: !0,
            button: 0,
            currentTarget: idleDisplay[0]
        };
        return idleDisplay[0].dispatchEvent(new MouseEvent("mouseover", eventOptions)), 
        delayUntil((function() {
            return "idle" !== getState();
        }), 2500)().ensure((function() {
            uiEventsHappening -= 1;
        }));
    }, showControls = function() {
        uiEventsHappening += 1;
        var scrubber, uiError = !1;
        try {
            if (!(scrubber = jQuery(".text-control"))) throw new Error;
            var eventOptions = {
                bubbles: !0,
                button: 0,
            ...
            console.log("controls not available"), uiEventsHappening -= 1, uiError = !0;
        }
        return delayUntil((function() {
            return scrubber && scrubber.is(":visible");
        }), 1e3)().ensure((function() {
            uiError || (uiEventsHappening -= 1);
        }));
    }, hideControls = function() {
        uiEventsHappening += 1;
        var player = jQuery(".VideoContainer"), eventOptions = {
            bubbles: !0,
            button: 0,
            ...
            offsetX: 100 - player.offset().left,
            offsetY: 100 - player.offset().top,
            pageX: 100,
            pageY: 100,
            currentTarget: player[0]
        };
        return player[0].dispatchEvent(new MouseEvent("mousemove", eventOptions)), delay(1)().ensure((function() {
            uiEventsHappening -= 1;
        }));
    }, play = function() {
        return uiEventsHappening += 1, jQuery(".button-nfplayerPlay").click(), delayUntil((function() {
            return "playing" === getState();
        }), 2500)().then(hideControls).ensure((function() {
            uiEventsHappening -= 1;
        }));
    }, seekErrorRecent = [];
    injectScript('window.seekScriptLoaded=!0;var getVideoPlayer=function(){var e=window.netflix.appContext.state.playerApp.getAPI().videoPlayer,t=e.getAllPlayerSessionIds()[0];return e.getVideoPlayerBySessionId(t)},seekInteraction=function(e){if(e.source==window){if(e.data.type&&"SEEK"==e.data.type)getVideoPlayer().seek(e.data.time);e.data.type&&"teardown"==e.data.type&&(window.removeEventListener("message",seekInteraction,!1),window.seekScriptLoaded=!1)}};window.addEventListener("message",seekInteraction,!1);');
    var socket, seek = function(milliseconds) {
        return function() {
            var newPlaybackPosition;
            logMessage(0, !0), uiEventsHappening += 1;
            var alreadyUpdated = !1;
            return window.postMessage({
                type: "SEEK",
                time: milliseconds
            }, "*"), delay(250)().then(delayUntil((function() {
                return alreadyUpdated || (alreadyUpdated = !0, socket.emit("buffering", {
                    buffering: !0
                }, (function() {}))), newPlaybackPosition = getPlaybackPosition(), "loading" !== getState();
            }), 1e4)).catch((function() {
                socket.emit("buffering", {
                    buffering: !1
                }, (function() {})), logMessage(0, !0);
            })).then((function() {
                socket.emit("buffering", {
                    buffering: !1
                }, (function() {}));
                var array, newSeekError = Math.min(Math.max(newPlaybackPosition - milliseconds, -1e4), 1e4);
    ...
    }, url = window.location.href, herokuSocket = !1;
    logMessage();
    getURLParameter(url, "npSessionId", 1);
    ...
    var serverId = npServerIdFromUrl || defaultServer, corsOptions = {
    ...
    npServerIdFromUrl ? {
        ...
        s798: !0,
        s799: !0
    }[npServerIdFromUrl] ? (socket = io("https://" + npServerIdFromUrl + ".netflixparty.com/", corsOptions), 
    ...
    var waitUserIdReady = function() {
        return console.log("wait user id ready called: "), function() {
            return new Promise((function(resolve, reject) {
                return delay(250)().then(delayUntil((function() {
                    ...
                    console.log("userId and userSettings are ready!"), permIdFix && (permId = userId), 
                    resolve();
                }));
            }));
        };
    ...
    var sessionStartTime, messages = [], unreadCount = 0, originalTitle = document.title, messagesCount = 0, interactionsCount = 0, summarySent = !1, typingTimer = null, allowFullScreen = function() {
        ...
        if (0 === jQuery("#chat-wrapper").length) {
            if (0 == jQuery(".sizing-wrapper").length || jQuery(".ltr-fntwn3").length > 0) {
                return showButtonMessage(testParticipationModal, "https://www.netflix.com/donottest"), 
                jQuery("body").after(chatHtml2), void teardown();
            }
            jQuery(".sizing-wrapper").after(chatHtml2), allowFullScreen(), jQuery("#presence-indicator").data("typing", !1), 
            ...
            jQuery("#link-icon").click((function(e) {
                if (console.log("linkIcon button clicked"), sessionId && serverId) {
            ...
            jQuery("#chat-wrapper").mousedown((function(e) {
                ...
                setBufferingPresenceVisible(data.anyoneBuffering), data.anyoneBuffering;
                var time = new Date;
        ...
        visible ? (jQuery(".sizing-wrapper").addClass("with-chat"), jQuery(".sizing-wrapper").css("right", "288px"), 
        jQuery("#chat-wrapper").show(), document.hasFocus() || clearUnreadCount()) : (jQuery("#chat-wrapper").hide(), 
        jQuery(".sizing-wrapper").removeClass("with-chat"), jQuery(".sizing-wrapper").css("right", "0px"));
        ...
        typing && buffering ? jQuery("#presence-indicator").text("People are typing and buffering...") : typing ? jQuery("#presence-indicator").text("People are typing...") : buffering ? jQuery("#presence-indicator").text("People are buffering...") : jQuery("#presence-indicator").text(" ");
    ...
    var oldSessionId = null, sessionDisabledStartTime = null, setSessionEnabled = function(enabled) {
        if (enabled) null != oldSessionId && null == sessionId && (sessionId = oldSessionId, 
        oldSessionId = null, sessionDisabledStartTime = null); else if (null == oldSessionId && null != sessionId) {
            oldSessionId = sessionId, sessionId = null;
            var now = new Date;
        ...
        (0 !== window.location.href.match(/^.*\/([0-9]+)\??.*/).length ? parseInt(window.location.href.match(/^.*\/([0-9]+)\??.*/)[1]) : null) != nextEpisode && (setSessionEnabled(!1), 
        simulClick = !0, jQuery(".WatchNext-still-hover-container").length > 0 ? jQuery(".WatchNext-still-hover-container").click() : jQuery(".button-nfplayerNextEpisode").length > 0 ? jQuery(".button-nfplayerNextEpisode").click() : jQuery(".nf-flat-button-text").length > 0 && jQuery(".nf-flat-button-text")[0].text.toLowerCase().includes("next episode") && jQuery(".nf-flat-button-text")[0].click());
    }, continueOtherEpisode = function(otherEpisode) {
        var currVideoId = 0 !== window.location.href.match(/^.*\/([0-9]+)\??.*/).length ? parseInt(window.location.href.match(/^.*\/([0-9]+)\??.*/)[1]) : null;
        showButtonMessage(currVideoId != otherEpisode ? longPartyRedirectModal : longPartyModal, getPartyUrl()), 
        teardown();
    };
    ...
    var replaceStateInteraction = function(event) {
        if (event.source == window && event.data.type && "FROM_PAGE" == event.data.type) {
            ...
            if (console.log(window.location.href), episodePage) {
                var nextVideoId = 0 !== window.location.href.match(/^.*\/([0-9]+)\??.*/).length ? parseInt(window.location.href.match(/^.*\/([0-9]+)\??.*/)[1]) : null;
                socket.emit("nextEpisode", {
                    ...
                    null != data && null != data.errorMessage && "Locked Session" == data.errorMessage && delayUntil((function() {
                        return nextVideoId == videoId;
                    }), 5e3)().catch((err => {
                        showButtonMessage(ownerOnlyNextEpisodeModal, getPartyUrl()), teardown();
                    }));
                })), otherEpisode ? (showButtonMessage(longPartyModal, getPartyUrl()), teardown()) : otherEpisode || (console.log("replace disable session next episode"), 
                ...
                swallow(delayUntil((function() {
                    ...
                    var remainingTimeLoaded = !1, remainingTime = getRemainingTime();
                    getRemainingTimeText();
                    return null != remainingTime && (remainingTimeLoaded = !0), remainingTimeLoaded && getPlaybackPosition() && "loading" !== getState();
                }), 1 / 0))).then((function() {
                    console.log("replace state loading DONE"), showControls();
                    var localTime = getPlaybackPosition(), scrubberHead = jQuery(jQuery(".scrubber-head")[0]);
                    parseInt(1e3 * scrubberHead.attr("aria-valuenow"));
                    showControls();
                    var now = new Date, newLastKnownTime = localTime, newLastKnownTimeUpdatedAt = new Date(now.getTime() - localTimeMinusServerTimeMedian), newState = "loading" === getState() ? "paused" : "playing" === getState() ? "playing" : "paused";
                    console.log("***********************************"), console.log("localTimeMinusServerTimeMedian: " + localTimeMinusServerTimeMedian), 
                    ...
                    if (console.log("RECENT UPDATE: " + recentUpdate), console.log("***********************************"), 
                    recentUpdate) {
                        console.log("recent update is true NO UPDATE"), console.log("re-enabling sync after next episode"), 
                        console.log("***********************************"), logState = !1, allowFullScreen(), 
                        setSessionEnabled(!0);
                    ...
                    state = newState, socket.emit(updateSessionTarget, {
                        ...
                        allowFullScreen(), setSessionEnabled(!0);
                        getPlaybackPosition(), "playing" === state && ((new Date).getTime(), lastKnownTimeUpdatedAt.getTime());
    ...
    var syncFromEnd = !1, updateSessionTarget = syncFromEnd ? "updateSessionFromEnd" : "updateSession", permIdFix = !1, sessionId = null, lastKnownTime = null, lastKnownTimeUpdatedAt = null, ownerId = null, state = null, videoId = null, videoDuration = null, currentPage = window.location.href, roundTripTimeRecent = [], roundTripTimeMedian = 0, localTimeMinusServerTimeRecent = [], localTimeMinusServerTimeMedian = 0, ping = function() {
        return new Promise((function(resolve, reject) {
        ...
        return null === sessionId ? Promise.resolve() : "paused" === state ? ("paused" === getState() ? promise = Promise.resolve() : (uiEventsHappening += 1, 
        jQuery(".button-nfplayerPause").click(), promise = delayUntil((function() {
            ...
            return Math.abs(localTime - serverTime) > 2500 ? seek(serverTime)().then((function() {
                ...
                function() {
                    return uiEventsHappening += 1, jQuery(".button-nfplayerPause").click(), delay(milliseconds)().then((function() {
    ...
    var broadcast = function(waitForChange) {
        return function() {
            ...
            getPlaybackPosition(), "playing" === state && ((new Date).getTime(), lastKnownTimeUpdatedAt.getTime());
            if (waitForChange) {
                var oldPlaybackPosition = getPlaybackPosition(), lastState = state;
            ...
            return promise.then(delayUntil((function() {
                getPlaybackPosition();
                var scrubberHead = jQuery(jQuery(".scrubber-head")[0]), scrubberTime = parseInt(1e3 * scrubberHead.attr("aria-valuenow"));
                showControls();
                var now = new Date, newLastKnownTime = scrubberTime, newLastKnownTimeRemaining = getDuration() - scrubberTime, newLastKnownTimeUpdatedAt = new Date(now.getTime() - localTimeMinusServerTimeMedian), newState = "loading" === getState() ? "paused" : "playing" === getState() ? "playing" : "paused";
        ...
        if (console.log("received data: " + JSON.stringify(data)), syncFromEnd) {
            if (getDuration() < data.lastKnownTimeRemaining) return pushTask(broadcast(!1)), 
            function() {
                return Promise.resolve();
            };
            lastKnownTime = getDuration() - data.lastKnownTimeRemaining;
        } else lastKnownTime = data.lastKnownTime;
        return state = data.state, lastKnownTimeUpdatedAt = new Date(data.lastKnownTimeUpdatedAt), 
        sync;
    }, tasks = null, tasksInFlight = 0, pushTask = function(task) {
        0 === tasksInFlight && (tasks = Promise.resolve()), tasksInFlight += 1, tasks = tasks.then((function() {
            "idle" === getState() && swallow(wakeUp)();
        })).then(swallow(task)).then((function() {
            tasksInFlight -= 1;
        }));
    }, mouseupListener = function() {
        if (console.log("mouseup"), target = event.target, jQuery(target).hasClass("button-nfplayerNextEpisode") ? (console.log("BUTTON NEXT EPISODE CLICK?:"), 
        1) : jQuery(target).hasClass("WatchNext-still-hover-container") || jQuery(target).hasClass("PlayIcon") ? (console.log("HOVER NEXT EPISODE CLICK"), 
        1) : jQuery(target).hasClass("button-nfplayerNextEpisode").length > 0 || jQuery(target).hasClass("nf-flat-button-text") && (console.log("CREDITS NEXT EPISODE CLICK?:" + jQuery(target).text()), 
        jQuery(target).text().toLowerCase().includes("next episode"))) console.log("sessionid disabled"), 
        setSessionEnabled(!1); else {
            var sessionIdString = sessionId || "null";
            console.log("sessionId, uiEventsHappening: " + sessionIdString + ", " + uiEventsHappening), 
            null !== sessionId && 0 === uiEventsHappening && tasksInFlight < 5 && pushTask((function() {
        ...
        console.log("keyup"), null !== sessionId && 0 === uiEventsHappening && tasksInFlight < 5 && pushTask((function() {
            return broadcast(!0)().catch(sync);
        }));
    };
    jQuery(window).mouseup(mouseupListener), jQuery(window).keyup(keyupListener);
    var scriptInterval = null, teardown = function() {
        ...
        jQuery("[tpInjected]").remove(), sessionId && (sessionId = null), window.telepartyLoaded = !1, 
        chrome.runtime.onMessage.removeListener(popupInteraction), socket && socket.disconnect(), 
        scriptInterval && clearInterval(scriptInterval), setChatVisible(!1), clearUnreadCount(), 
        jQuery("#chat-container").remove(), jQuery("#chat-wrapper").remove(), jQuery(".sizing-wrapper").removeClass("with-chat"), 
        jQuery(".sizing-wrapper").css("right", "0px"), logState = !1;
    };
    socket.on("connect", (function() {
        pushTask(ping), console.log(currentPage);
        new Date;
        scriptInterval || (scriptInterval = setInterval((function() {
            if (0 === tasksInFlight) {
                new Date;
        ...
        null !== sessionId && socket.emit("reboot", {
            ...
            teardown()) : (logMessage(), pushTask(receive(data)));
        }));
    })), socket.on("update", (function(data) {
        pushTask(receive(data));
    }));
    var popupInteraction = function(request, sender, sendResponse) {
        if ("getInitData" === request.type) {
            version = request.data.version;
            var currVideoId = 0 !== window.location.href.match(/^.*\/([0-9]+)\??.*/).length ? parseInt(window.location.href.match(/^.*\/([0-9]+)\??.*/)[1]) : null, videoDomId = void 0 !== jQuery("video").parent()[0] ? parseInt(jQuery("video").parent()[0].id) : null, tempVideoId = request.data.videoId;
            return null != videoDomId && videoDomId != currVideoId && (history.replaceState("data to be passed", "Title of the page", "/watch/" + jQuery(jQuery("video").parent())[0].id), 
            tempVideoId = videoDomId), void sendResponse({
                sessionId,
                serverId,
                videoId: tempVideoId,
        ...
        return "createSession" === request.type ? (waitUserIdReady()().then((function() {
            console.log("create session permId: " + permId), console.log("create session permId: " + JSON.stringify(userSettings)), 
            socket.emit("createSession", {
                ...
                data.errorMessage ? sendResponse({
                ...
                messages = [], sessionId = data.sessionId, !1, ownerId = request.data.controlLock ? userId : null, 
        ...
        })), !0) : "joinSession" === request.type ? (waitUserIdReady()().then((function() {
            ...
            socket.emit("joinSession", joinData, (function(data) {
                if (data.errorMessage) sendResponse({
                    ...
                    initChat(), setChatVisible(!0), sessionId = request.data.sessionId, lastKnownTime = data.lastKnownTime, 
                    syncFromEnd = !!data.syncFromEnd && data.syncFromEnd, updateSessionTarget = syncFromEnd ? "updateSessionFromEnd" : "updateSession", 
                    syncFromEnd && (lastKnownTime = getDuration() - data.lastKnownTimeReamining), lastKnownTimeUpdatedAt = new Date(data.lastKnownTimeUpdatedAt), 
                    messages = [];
                    for (var i = 0; i < data.messages.length; i += 1) addMessage(data.messages[i], !0);
                    ownerId = data.ownerId, null != data.ownerId && data.ownerId !== userId, state = data.state, 
                    videoId = request.data.videoId, videoDuration = getDuration(), pushTask(receive(data)), 
                    sendResponse({});
                }
            }));
        })), !0) : "leaveSession" === request.type ? (socket.emit("leaveSession", null, (function(_) {
            logSummary(), sessionId = null, !1, setChatVisible(!1), sendResponse({});
    ...
    (console.log("user Id promise called: " + userId), delayUntil((function() {
        ...
        return console.log("fix, userId getChromeStorage: " + permIdFix + ", " + userId), 
        new Promise((function(resolve, reject) {
            userSettings.userId && userSettings.userIcon && (resolve(userSettings), console.log("fix, userId resolve getChromeStorage: " + permIdFix + ", " + userId)), 
            chrome.storage.local.get(null, (function(data) {
                data.userId || (data.userId = userId, console.log("fix, userId CALLED getChromeStorage: " + permIdFix + ", " + userId), 
                permId = userId), console.log("icons:" + JSON.stringify(oldIcons)), console.log("user icon:" + JSON.stringify(data.userIcon));
                var userIconFix = !1;
        ...
        if (jQuery(".ltr-fntwn3").length > 0) {
            showButtonMessage(testParticipationModal, "https://www.netflix.com/donottest");
        }
