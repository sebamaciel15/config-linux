var injectContentScript = function() {
    ...
    }, delayUntil = function(condition, maxDelay, delayStep = 250) {
        return function(result) {
            var startTime = (new Date).getTime(), checkForCondition = function() {
                return condition() ? Promise.resolve(result) : null !== maxDelay && (new Date).getTime() - startTime > maxDelay ? Promise.reject(Error("delayUntil timed out")) : delay(delayStep)().then(checkForCondition);
            };
    ...
    }, uiEventsHappening = 0, changingVideo = !1, videoChangeStartTime = null, lastDuration = 36e5, getDuration = function() {
        var video = jQuery("video");
        return video && video.length > 0 && (lastDuration = Math.floor(1e3 * video[0].duration)), 
        lastDuration;
    }, getState = function() {
        return null == video ? "none" : video.readyState < video.HAVE_FUTURE_DATA ? "loading" : video.paused ? "paused" : "playing";
    }, getPlaybackPosition = function() {
        return video ? Math.floor(1e3 * video.currentTime) : null;
    }, getRemainingTime = function() {
        return video ? Math.floor(1e3 * (video.duration - video.currentTime)) : null;
    ...
    function clickAtX(target, x, eventType) {
    ...
    async function seekToCurrentTime(time) {
        var video = document.querySelector("video");
        video.click(), await delay(130)();
        var seekbar = document.querySelector(".progress-bar .slider-container"), x = function(progress) {
            var seekbar = document.querySelector(".progress-bar .slider-container"), seekBounds = seekbar.getBoundingClientRect();
            return progress * seekbar.offsetWidth + seekBounds.left;
        }(time / (1e3 * video.duration));
        clickAtX(seekbar, x, "pointerdown"), clickAtX(seekbar, x, "pointerup");
    }
    var socket, seek = function(milliseconds) {
        return logMessage(0, !0), new Promise((async function(resolve, reject) {
            uiEventsHappening += 1;
            try {
                await seekToCurrentTime(milliseconds), await delayUntil((() => "loading" !== getState()), 1e4)(), 
                resolve();
            } catch (error) {
                console.error("Seek took too long to complete. Video must be buffering"), reject(error);
            } finally {
                uiEventsHappening -= 1;
            }
        }));
        ...
        return new Promise((async function(resolve, reject) {
            uiEventsHappening += 1;
            try {
                jQuery(".btm-media-player").click(), await delay(100)(), jQuery(".play-icon").length > 0 && jQuery(".play-icon").click(), 
                await delayUntil((() => "playing" === getState()), 1e3)(), resolve();
            } catch (error) {
                "playing" === getState() && resolve(), reject(error);
            } finally {
                uiEventsHappening -= 1;
            }
        }));
        ...
        return new Promise((async function(resolve, reject) {
            uiEventsHappening += 1;
            try {
                jQuery(".btm-media-player").click(), await delay(100)(), jQuery(".pause-icon").length > 0 && jQuery(".pause-icon").click(), 
                await delayUntil((() => "paused" === getState()), 1e3)(), resolve();
            } catch (error) {
                reject(error);
            } finally {
                uiEventsHappening -= 1;
    ...
    logMessage();
    var defaultServer = defaultServerOptions[Math.floor(Math.random() * defaultServerOptions.length)], servers = {
        load: !0,
        undefined: !0,
        wpseth: !0,
        sundefined: !0,
    ...
    const npServerIdFromUrl = function(url, key, queryIndex) {
    ...
    }(url, "npServerId", 1), serverId = npServerIdFromUrl || defaultServer;
    ...
    var sessionStartTime, messages = [], unreadCount = 0, originalTitle = document.title, messagesCount = 0, summarySent = !1, typingTimer = null, isChatVisible = !0, linkIconListener = function(e) {
    ...
    const wrapperObserver = new MutationObserver((function(mutationsList, observer) {
        for (let mutation of mutationsList) "class" == mutation.attributeName && mutation.target.classList.contains("video_view--theater") && setChatVisible(isChatVisible);
    })), loadingObserver = new MutationObserver((function(mutationsList, observer) {
        for (let mutation of mutationsList) {
            if (mutation.addedNodes && mutation.addedNodes.length > 0) jQuery(mutation.addedNodes).find(".loading-icon").length && onBuffer();
            if (mutation.removedNodes && mutation.removedNodes.length > 0) jQuery(mutation.removedNodes).find(".loading-icon").length && videoCanPlay();
        }
    }));
    var setupObservers = function() {
        ...
        wrapperObserver.disconnect(), loadingObserver.disconnect(), targetNode && wrapperObserver.observe(targetNode, config), 
        overlayContainer && loadingObserver.observe(overlayContainer, {
            childList: !0
        });
    }, setupChatHandlers = function() {
        jQuery(".user-icon").click(largeUserIconListener), jQuery("#user-icon").click(userIconListener), 
        jQuery("#link-icon").click(linkIconListener), Object.keys(iconMap).forEach((function(categoryName) {
        ...
        })), jQuery(".image-button").click(userIconSelectorListener), jQuery("#chat-wrapper").keydown(onChatKeyDown), 
        jQuery("#chat-input").keyup(onChatKeyPressed), jQuery(".btns button").click(saveChangesListener), 
        jQuery("#chat-input-container").click((() => jQuery("#chat-input").focus()));
    }, initChat = function() {
        0 === jQuery("#chat-wrapper").length ? (jQuery("#hudson-wrapper").after(chatHtml2), 
        setupChatHandlers(), setupObservers()) : jQuery("#chat-history").html(""), jQuery("#presence-indicator").data("typing", !1), 
        ...
        isChatVisible = visible, visible ? (jQuery("#hudson-wrapper").addClass("with-chat"), 
    ...
    function loadNewVideo() {
        ...
        return new Promise((async function(resolve, reject) {
            var interval = setInterval((() => {
                if (window.location.href.match(/^.*\/([0-9a-zA-Z-]+)\??.*/)[1] === videoId) {
                    var videoElement = document.querySelector("video");
                    videoElement instanceof Element && videoElement.src && videoElement.readyState > 0 && (setupObservers(), 
                    clearInterval(interval), loadVideoHandlers(videoElement), changingVideo = !1, delayUntil((() => "loading" !== getState()), 1 / 0)().then((() => {
    ...
    var goToNextEpisode = function() {
        return new Promise((async function(resolve, reject) {
            ...
            if (changingVideo = !0, currentVid != videoId) {
                video.currentTime = video.duration;
                (new Date).getTime();
                await delayUntil((() => document.querySelector(".play") instanceof Element), 2e3)().then((async () => {
                    (new Date).getTime();
                    logMessage(), await delay(500)(), jQuery(".play").click(), pushTask(loadNewVideo);
                })).catch((() => {
                    logMessage(), showButtonMessage(failedNextEpisodeModal, getPartyUrl()), teardown();
                }));
            } else pushTask(loadNewVideo);
            uiEventsHappening -= 1, resolve();
        }));
    };
    socket.on("jumpToNextEpisode", (data => {
        videoId = data.nextEpisode, videoChangeStartTime = new Date((new Date).getTime() - localTimeMinusServerTimeMedian), 
        pushTask(goToNextEpisode);
    }));
    var script, s;
    console.log("replace script loaded: " + window.replaceScriptLoaded), window.replaceScriptLoaded || (script = '\n    if(!window.replaceScriptLoaded) {\n        window.replaceScriptLoaded = true;\n      (function(history){\n        var replaceState = history.replaceState;\n        history.replaceState = function(state) {\n          if (typeof history.onreplacestate == "function") {\n            history.onreplacestate({state: state});\n          }\n          return replaceState.apply(history, arguments);\n        }\n        var pushState = history.pushState;\n        history.pushState = function(state) {\n            if (typeof history.onpushstate == "function") {\n                history.onpushstate({state: state});\n            }\n            return pushState.apply(history, arguments);\n        };\n      })(window.history);\n\n      var popInteraction = function(e) {\n        // send message to content script w next episode\n        window.postMessage({ type: "FROM_PAGE_POP", text: "next episode from the webpage!"}, "*");\n      }\n\n      var reloadInteraction = function(e) {\n        // send message to content script w next episode\n        window.postMessage({ type: "FROM_PAGE", text: "next episode from the webpage!"}, "*");\n      }\n      window.onpopstate = popInteraction;\n      history.onreplacestate = history.onpushstate = reloadInteraction;\n    }\n  ', 
    (s = document.createElement("script")).setAttribute("tpInjected", ""), s.textContent = script, 
    ...
    var replaceStateInteraction = function(event) {
        if (event.source == window) if ("FROM_PAGE_POP" !== event.data.type) {
            if (event.data.type && "FROM_PAGE" == event.data.type) {
                console.log("***********************************"), console.log("Content script received: " + event.data.text);
                var episodePage = window.location.href.match(/^.*\/(video)\/.*/);
                if (console.log(window.location.href), !episodePage && !changingVideo) return logMessage(), 
                void teardown();
                logMessage();
                var parseIds = window.location.href.match(/^.*\/([a-z\-0-9]+)\??.*/), nextVideoId = parseIds.length > 1 ? parseIds[1] : null;
                if (null == nextVideoId) return logMessage(), void teardown();
                nextVideoId != videoId && socket.emit("nextEpisode", {
    ...
    const permIdFix = !1;
    var sessionId = null, lastKnownTime = null, lastKnownTimeUpdatedAt = null, lastUpdateEvent = null, ownerId = null, hostOnly = !1, state = null, videoId = null, videoDuration = null, roundTripTimeRecent = (window.location.href, 
    []), roundTripTimeMedian = 0, localTimeMinusServerTimeRecent = [], localTimeMinusServerTimeMedian = 0, ping = function() {
        ...
        hostOnly ? pushTask(sync) : uiEventsHappening <= 0 && sessionId && pushTask(broadcast);
    }, onBuffer = function() {
        if (sessionId && !changingVideo) {
            pushTask((function() {
        ...
        (video = src).onplay = onVideoChange, video.onpause = onVideoChange, video.onseeking = onVideoChange;
    }, sync = function() {
        return null === sessionId || uiEventsHappening > 0 || changingVideo ? Promise.resolve() : delayUntil((function() {
            return "loading" !== getState();
        }), 1 / 0, 10)().then((function() {
            if ("paused" === state) {
            ...
            return "paused" === getState() && (promise = play()), promise.then((function() {
                if (Math.abs(localTime - serverTime) > 2500) return seek(serverTime).then((function() {
                    ...
                    new Promise((async function(resolve, reject) {
                        uiEventsHappening += 1;
                        try {
                            await pause(), await delay(milliseconds)(), await play();
                        } catch (error) {
                            reject(error);
                        } finally {
                            uiEventsHappening -= 1;
        ...
        return new Promise((async (resolve, reject) => {
            ...
            var newState, bufferingState = "loading" == getState(), now = new Date, localTime = getPlaybackPosition(), serverTime = lastKnownTime + ("playing" === state ? now.getTime() - (lastKnownTimeUpdatedAt.getTime() + localTimeMinusServerTimeMedian) : 0), newLastKnownTime = localTime, newLastKnownTimeUpdatedAt = new Date(now.getTime() - localTimeMinusServerTimeMedian);
            newState = "loading" == getState() ? video.paused ? "paused" : "playing" : "playing" === getState() ? "playing" : "paused", 
            state === newState && Math.abs(localTime - serverTime) < 1 ? resolve() : socket.emit("updateSession", {
                ...
                void 0 !== data && null !== data.errorMessage ? reject() : resolve();
            }));
        }));
    }, receive = function(data) {
        return lastKnownTime = data.lastKnownTime, state = data.state, lastKnownTimeUpdatedAt = new Date(data.lastKnownTimeUpdatedAt), 
        sync;
        ...
        tasks = Promise.resolve(), scriptInterval && clearInterval(scriptInterval), wrapperObserver.disconnect(), 
        loadingObserver.disconnect(), setChatVisible(!1), clearUnreadCount(), jQuery("#chat-container").remove(), 
        jQuery("#chat-wrapper").remove(), jQuery("#hudson-wrapper").removeClass("with-chat");
    };
    socket.on("connect", (function() {
        ...
        null != sessionId && (logMessage(), socket.emit("reboot", {
            ...
            data.errorMessage ? (showButtonMessage(lostConnectionModal, getPartyUrl()), teardown()) : data.videoIds.pop() !== videoId ? (showButtonMessage(failedNextEpisodeModal, getPartyUrl()), 
    ...
    var popupInteraction = function(request, sender, sendResponse) {
        return "getInitData" === request.type ? (video ? sendResponse({
            sessionId,
            serverId,
            chatVisible: getChatVisible()
        }) : delayUntil((() => document.querySelector("video") instanceof Element), 5e3)().then((() => {
            loadVideoHandlers(document.querySelector("video")), version = request.data.version, 
            sendResponse({
                sessionId,
                serverId,
                chatVisible: getChatVisible()
        ...
        })), !0) : "createSession" === request.type ? (waitUserIdReady()().then((function() {
            console.log("create session permId: " + permId), socket.emit("createSession", {
                ...
                videoId = request.data.videoId, videoDuration = getDuration(), pushTask(broadcast), 
                sessionStartTime = new Date, sendResponse({
                    sessionId
                }));
            }));
            ...
            socket.emit("joinSession", joinData, (function(data) {
                if (data.errorMessage) sendResponse({
                    ...
                    lastKnownTimeUpdatedAt = new Date(data.lastKnownTimeUpdatedAt), lastUpdateEvent = new Date(data.lastKnownTimeUpdatedAt), 
                    messages = [];
                    ...
                    pushTask(receive(data)), sendResponse({});
                }
            }));
        })).catch((() => {
            sendResponse({
            ...
            logSummary(), sessionId = null, hostOnly = !1, setChatVisible(!1), sendResponse({});
